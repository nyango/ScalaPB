...
class SealedOneofsCache(values: Seq[SealedOneof]) {
  private val containerToCases: Map[Descriptor, Seq[Descriptor]] = values.map(s => s.container -> s.cases).toMap
  private val caseToContainer: Map[Descriptor, Descriptor] = values.flatMap(s => s.cases.map(_ -> s.container)).toMap
